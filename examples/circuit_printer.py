...
from reporting import make_run_dir, print_report_banner, tee_stdout, write_run_summary
...
def print_circuit_details(circuit, input_state, measurement_mode, title):
    ...
    if "clements" in circuit.name.lower():
        ...
        print(
            f"  2. Mesh of {circuit.m * (circuit.m - 1) // 2} MZIs in rectangular grid pattern"
        )
        print("  3. Each MZI has two phase shifters (internal and external)")
        print(
            f"  4. Output: Measure probability of photon in mode {measurement_mode[0]}"
        )
...
def create_clements_with_memristive(n_modes=3, memristive_phase_idx=None):
    ...
    print_circuit_details(
        ...
        input_state,
        measurement_mode,
        f"Clements {n_modes}x{n_modes} (standalone)",
    )
    print_circuit_details(
        full,
        input_state,
        measurement_mode,
        f"Clements {n_modes}x{n_modes} with Encoding",
    )
    return full
...
def create_clements_circuit(n_modes=6):
    ...
    print_circuit_details(
        ...
        input_state,
        measurement_mode,
        f"Clements Circuit {n_modes}x{n_modes} (standalone)",
    )
    print_circuit_details(
        full_clements,
        input_state,
        measurement_mode,
        f"Clements Circuit {n_modes}x{n_modes} with Encoding",
    )

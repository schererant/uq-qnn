...
def main():
    ...
    plt.subplot(2, 3, 4)
    scatter = plt.scatter(
        X_test, entropy, c=(mean_preds != y_test_labels), cmap="RdYlGn", alpha=0.7
    )
    ...
    plt.tight_layout()
    plt.savefig(report_dir / "classification_with_uncertainty.png", dpi=300)
    plt.show()

    write_run_summary(
        report_dir,
        metrics={"test_accuracy": float(accuracy)},
        artifacts=["classification_with_uncertainty.png"],
    )

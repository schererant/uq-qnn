#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Coincidence regression with a memristive phase (chip-output feedback)."""
...
CONFIG = {
    ...
    "memristive_output_modes": MEMRISTIVE_FEEDBACK_PAIR,
    "output_mode": "coincidence",
    "working_detectors": tuple(range(6)),
    "feedback_mode": "chip_output",
    "feedback_modes": MEMRISTIVE_FEEDBACK_PAIR,
    "loss_type": "mse",
    "n_classes": 1,
    "lr": 0.05,
    "epochs": 80,
...
def _plot_training_loss(history: np.ndarray) -> plt.Figure:
    ...
    ax.plot(history, color="tab:red", lw=2)
    ax.set_yscale("log")
    ax.set(
        xlabel="Epoch",
        ylabel="Loss",
        title=(
            "Memristive Training Loss: input_state=%s, CC%s%s, encoding_phase_idx=%d"
...
def _plot_summary(
    ...
    axes[0, 0].plot(history, color="tab:red")
    ...
    axes[0, 1].set(
        ...
        ylabel="y",
        title=(
            "Memristive coincidence regression\n"
...
def main() -> None:
    with Experiment("Coincidence Regression (Memristor)", config=CONFIG) as exp:
        ...
        logger.info(
            "Training memristive coincidence regression with input_state=%s, "
            "target_mode=%s, encoding_phase_idx=%d, memristive_phase_idx=%s",
            INPUT_STATE,
            TARGET_CC_PAIR,
            ENCODING_PHASE_IDX,
        ...
        exp.save_metrics(
            {
                ...
                "feedback_pair": MEMRISTIVE_FEEDBACK_PAIR[0],
                "input_state": INPUT_STATE,
                "encoding_phase_idx": ENCODING_PHASE_IDX,
                "memristive_phase_idx": MEMRISTIVE_PHASE_IDX,
        ...
        exp.savefig(
            ...
            "coincidence_regression_memristor_with_uncertainty.png",
        )

        logger.info(
            "Memristive coincidence regression: final_loss=%.6f mse=%.6f rmse=%.4f r2=%.4f",
            final_loss,
            metrics["mse"],
            metrics["rmse"],
            metrics["r2"],

...
def train_and_evaluate(datafunction, n_data=80, sigma_noise=0.05, n_samples=500, epochs=30):
    """
...
def plot_function_comparison(results, functions):
    ...
    for i, func_name in enumerate(functions):
        ...
        ax = axes[i, 2]
        abs_error = np.abs(mean_preds - y_test)
        ax.scatter(std_preds, abs_error, alpha=0.7)
...
def main():
    ...
    for func_name in functions:
        ...
        X_train, y_train, X_test, y_test, mean_preds, std_preds, theta_opt, history = train_and_evaluate(
            func_name, n_data, sigma_noise, n_samples, epochs
        )

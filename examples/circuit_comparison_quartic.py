...
"""
Circuit Comparison with Quartic Function Example

This script demonstrates:
1. Creating a memristor circuit and a 6-mode Clements circuit
2. Visualizing both circuits with Perceval's circuit printing
3. Training both circuits on the quartic function
...
def visualize_circuit(circuit, input_state, measurement_mode, title):
    ...
    # Render circuit using Perceval's representation
...
def create_memristor_circuit():
    ...
    visualize_circuit(
        mem_circuit, input_state, measurement_mode, "Memristor Circuit (3 modes)"
    )
...
def train_and_evaluate(
    label, n_modes, X_train, y_train, X_test, y_test, memristive_phase_idx=None
):
    ...
    if memristive_phase_idx:
        print(
            f"Clements {n_modes}x{n_modes} with memristive phases {memristive_phase_idx}"
        )
    else:
...
def plot_results(results, X_train, y_train, X_test, y_test, report_dir):
    ...
    ax.set_xlabel("x")
    ax.set_ylabel("y")
    ax.set_title("Predictions on Quartic Function")
    ax.legend()
    ax.grid(True)
    ...
    plt.savefig(report_dir / "circuit_quartic_comparison.png", dpi=300)
...
def main():
    ...
    write_run_summary(
        ...
        artifacts=[
            ...
            "circuit_quartic_comparison.png",
        ],
    )

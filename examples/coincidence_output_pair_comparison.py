...
def _sim_cfg(output_pair: Tuple[int, int]) -> SimConfig:
    cfg = {**CONFIG, "target_mode": output_pair}
...
def plot_metrics_comparison(results: Dict, pairs: List[Tuple[int, int]]) -> plt.Figure:
    ...
    fig.suptitle(f"Performance Metrics by Output CC Pair — {FUNC_LABEL}", fontsize=12)
    fig.tight_layout()
    return fig
...
def main() -> None:
    with Experiment("Coincidence Output Pair Comparison", config=CONFIG) as exp:

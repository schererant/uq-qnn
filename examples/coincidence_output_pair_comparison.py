...
# -*- coding: utf-8 -*-
"""
Coincidence Output-Pair Comparison Experiment.
...
output CC pairs for a 6-mode Clements mesh.

Each run injects two photons at fixed input_modes=(1, 4) and reads a
different postselected coincidence channel as the scalar regression
output, controlled via target_mode=(j, k).  All 6 detectors remain
working so the full 15-channel normalization context is preserved.

This isolates the effect of *which coincidence channel* is most
expressive for a given function, while the input state stays fixed.

Produced artefacts
------------------
predictions_grid.png         Fit + 95 % CI for every output CC pair
training_loss.png            Log-scale convergence, one curve per CC pair
calibration.png              σ vs |error| scatter, all CC pairs overlaid
metrics_comparison.png       Bar charts: RMSE, R², coverage@95 %, NLL
r2_bar.png                   R² sorted best → worst
uncertainty_distribution.png Violin plots of per-point σ per CC pair
...
from src.coincidence import get_cc_mode_pairs
...
CC_PAIRS: List[Tuple[int, int]] = get_cc_mode_pairs(N_MODES)
...
_CMAP = matplotlib.colormaps["tab20"].resampled(len(CC_PAIRS))
PAIR_COLORS: Dict[Tuple[int, int], object] = {
    p: _CMAP(i) for i, p in enumerate(CC_PAIRS)
}
...
def _sim_cfg(output_pair: Tuple[int, int]) -> SimConfig:
    cfg = {**_COMMON, "target_mode": output_pair}
...
def train_and_evaluate(
    ...
    j, k = output_pair
    logger.info("▶ Training  output CC pair=%s", _pair_label(j, k))
    sc = _sim_cfg(output_pair)
...
def plot_predictions_grid(
    ...
    fig.suptitle(
        ...
        f"All output CC pairs  (input_modes={INPUT_MODES})",
        fontsize=13,
        y=1.01,
    )
    fig.tight_layout()
...
def plot_metrics_comparison(
    ...
    fig.suptitle(
        f"Performance Metrics by Output CC Pair — {FUNC_LABEL}", fontsize=12
    )
    fig.tight_layout()
    return fig
...
def plot_r2_bar(results: Dict, pairs: List[Tuple[int, int]]) -> plt.Figure:
    ...
    ax.set_xlabel("R²")
    ax.set_title(
        f"R² by Output CC Pair — {FUNC_LABEL}\n"
        f"(sorted best → worst, input_modes={INPUT_MODES})",
        fontsize=10,
    )
...
def plot_uncertainty_distribution(
    ...
    ax.set_title(
        f"Uncertainty Distribution by Output CC Pair — {FUNC_LABEL}",
        fontsize=10,
    )
    ax.grid(True, axis="y", alpha=0.25)
    fig.tight_layout()
...
def plot_metrics_table(
    ...
    ax.set_title(
        f"Metrics Summary — {FUNC_LABEL}  (input_modes={INPUT_MODES})",
        fontsize=11,
        pad=18,
    )
    fig.tight_layout()
...
def main() -> None:
    base_cfg = {**_COMMON, "target_mode": CC_PAIRS[0]}

    with Experiment(
        "Coincidence Output Pair Comparison", config=base_cfg
    ) as exp:
        np.random.seed(_COMMON["seed"])

        X_train, y_train, X_test, y_test = get_data(
            N_DATA, SIGMA_NOISE, FUNCTION
        )
        logger.info(
        ...
        logger.info(
            "Sweeping %d output CC pairs with input_modes=%s",
            len(CC_PAIRS),
            INPUT_MODES,
        )
        ...
        for pair in CC_PAIRS:
            ...
            results[pair] = r
            all_metrics[f"CC{j}{k}"] = r["metrics"]

        exp.save_metrics({"by_output_cc_pair": all_metrics})
        ...
        figs: List[Tuple[str, plt.Figure]] = [
            ...
            ("training_loss.png", plot_training_loss(results, CC_PAIRS)),
            (
                "calibration.png",
                plot_calibration(results, y_test, CC_PAIRS),
            ),
            (
                "metrics_comparison.png",
                plot_metrics_comparison(results, CC_PAIRS),
            ),
            ("r2_bar.png", plot_r2_bar(results, CC_PAIRS)),
            (
                "uncertainty_distribution.png",
                plot_uncertainty_distribution(results, CC_PAIRS),
            ),
            ("metrics_table.png", plot_metrics_table(results, CC_PAIRS)),
        ]

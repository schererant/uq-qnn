...
def train_mega_memristor(X_train, 
                    y_train, 
                    memory_depth, 
                    training_steps, 
                    learning_rate, 
                    ...
                    plot_path: str = None):
    """
    Trains the longer memristor model using the training data.
        ...
        with tf.GradientTape() as tape:
            ...
            for i in tqdm(range(num_samples), 
                ...
                if i == 0:
                    ...
                    memristor_phase = tf.acos(tf.sqrt(
                        tf.reduce_sum(memory_p3) / memory_depth +
                        memristor_weight * tf.reduce_sum(memory_p2) / memory_depth
                    ))
                    memristor_circuit = MemristorMegaCircuit(phase1, memristor_phase, phase3, phase4, memristor2_weight, phase6, encoded_phases[i])
                    results = eng.run(
...
def predict_mega_memristor(X_test: np.ndarray, 
                      y_test: np.ndarray, 
                      memory_depth: int, 
                      phase1: float, 
                      phase3: float, 
                      ...
                      param_id: str = None):
    """
    Uses the trained longer memristor model to make predictions on test data.
    """

...
def parse_arguments():
    ...
    circuit_group.add_argument(
        "--encoding-mode", type=int, default=0, help="Mode to apply encoding to"
    )
    circuit_group.add_argument(
        "--target-mode",
        type=str,
    ...
    swipe_group.add_argument(
        "--continuous", action="store_true", help="Use continuous-swipe training"
    )
    swipe_group.add_argument(
        "--n-swipe",
        type=int,
    ...
    swipe_group.add_argument(
        "--f-laser-khz", type=float, default=50.0, help="Laser repetition rate in kHz"
    )
    swipe_group.add_argument(
        "--det-window-us",
        type=float,
    ...
    swipe_group.add_argument(
        "--max-swipe", type=int, default=21, help="Maximum allowed swipe count"
    )

    # Training options
    train_group = parser.add_argument_group("Training")
    train_group.add_argument("--lr", type=float, default=0.03, help="Learning rate")
    ...
    output_group.add_argument("--no-plot", action="store_true", help="Disable plotting")
    output_group.add_argument(
        "--save-model",
        type=str,
        default=None,
...
def update_config_from_args(args):
    ...
    if args.target_mode:
        try:
            ...
            if any(m >= args.n_modes for m in target_mode):
                print(
                    "Warning: Target mode exceeds available modes in Clements circuit"
                )
                target_mode = (min(args.n_modes - 1, max(0, target_mode[0])),)
        except ValueError:
            print(
                f"Warning: Invalid target mode format '{args.target_mode}', using default"
            )
            target_mode = (args.n_modes - 1,)
    ...
    config["input_modes"] = (
        tuple(int(m) for m in args.input_modes.split(",")) if args.input_modes else None
    )
    config["working_detectors"] = (
        (
            tuple(int(m) for m in args.working_detectors.split(","))
            if args.working_detectors
            else (0, 1, 5)
            if args.n_modes >= 6
            else (0, 1)
        )
...
def run_training(X_train, y_train, X_test, y_test, args):
    ...
    if config["do_plot"]:
        plot_results(
            X_train, y_train, X_test, y_test, preds, history, continuous_mode, args
        )
...
def plot_results(
    ...
    if continuous_mode:
        enc_swipe, _ = get_cont_swipe_data(
            X_train, y_train, n_swipe=config["n_swipe"], swipe_span=config["swipe_span"]
        )
    ...
    mem_str = (
        f", memristive={args.memristive_phase_idx}" if args.memristive_phase_idx else ""
    )
    circuit_info = (
        f"Clements {args.n_modes}x{args.n_modes}: {args.n_phases} phases{mem_str}"
    )
...
def main():
    ...
    print(
        f"Encoding mode: {args.encoding_mode}, Target mode(s): {config['target_mode']}"
    )
    if args.encoding_mode >= args.n_modes:
        print(
            f"Warning: Encoding mode {args.encoding_mode} exceeds available modes ({args.n_modes}), setting to 0"
        )
    ...
    if args.measured_data:
        ...
        print(
            f"Generating synthetic data with {args.n_data} points and noise σ={args.sigma_noise}"
        )
        print(f"Using data function: {config['datafunction']}")
        X_train, y_train, X_test, y_test = get_data(
            config["n_data"], config["sigma_noise"], config["datafunction"]
        )

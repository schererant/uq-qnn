...
def model_comparison(X_train, y_train, X_test, y_test):
    ...
    for hidden_layers in mlp_architectures:
        ...
        metrics, metric_categories = compute_eval_metrics(
            np.array(mlp_predictions), np.array(y_test), np.array([])
        )
    ...
    with open(config.log_file_name, "a") as f:
        ...
        for hidden_layers in mlp_architectures:
            ...
            f.write("-" * 40 + "\n")
            f.write("\nMetrics:\n")
            metric_lines = format_metrics(
                all_results[f"mlp_{len(hidden_layers)}_layers"]["metrics"], indent=2
            )
            f.write("\n".join(metric_lines))
            f.write("\n\n")
    ...
    for n_samples in config.model_comparison.n_samples:
        ...
        full_metrics, full_metric_categories = compute_eval_metrics(
            predictions, targets, predictive_uncertainty
        )
        ...
        for threshold in [0.7]:
            ...
            sel_predictions, sel_targets, sel_uncertainty, remaining_fraction = (
                selective_prediction(
                    predictions, targets, predictive_uncertainty, threshold=threshold
                )
            )

            sel_metrics, sel_metric_categories = compute_eval_metrics(
                sel_predictions, sel_targets, sel_uncertainty
            )

...
def hyperparameter_optimization(X_train, y_train, X_test, y_test):
    ...
    for steps, learning_rate, memory_depth, cutoff_dim in tqdm(
        product(
            Config.HYPER_STEPS_RANGE,
            Config.HYPER_LEARNING_RATE_RANGE,
            Config.HYPER_MEMORY_DEPTH_RANGE,
            Config.HYPER_CUTOFF_DIM_RANGE,
        ),
        total=len(Config.HYPER_STEPS_RANGE)
        * len(Config.HYPER_LEARNING_RATE_RANGE)
        * len(Config.HYPER_MEMORY_DEPTH_RANGE)
        * len(Config.HYPER_CUTOFF_DIM_RANGE),
        ...
        if Config.PREDICT_STOCHASTIC:
            ...
            sel_predictions, sel_targets, sel_uncertainty, remaining_fraction = (
            ...
            plot_predictions(
                ...
                y_test.numpy(),
                predictions,
                pred_std=predictive_uncertainty,
                epistemic=predictive_uncertainty,
                aleatoric=None,
                title="Memristor Model Predictions vs Targets",
                save_path=Config.LOG_PATH
                + f"prediction_uncertainty_{Config.PARAM_ID}.png",
            )
    ...
    with open(Config.LOG_FILE_NAME, "a") as f:
        ...
        if "best_parameters" in all_results:
            ...
            f.write("\nHyperparameters:\n")
            hyperparameter_lines = format_hyperparameters(
                result["hyperparameters"], indent=2
            )

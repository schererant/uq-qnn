    ...
    for idx, (label, phase_data) in enumerate(data_dict.items()):
        ...
        try:
            start_index = next(
                ...
                if time[j] > 0.02 and phase_data[j] < avg_pre - threshold
            )
        ...
        end_index = next(
            j
            for j in range(start_index + 1, len(phase_data))
            if phase_data[j] <= avg_post
        )
...
summary_part = analyze_phase_shifts(
    phase_data_dic, figure_title="Phase Shifts (First 10 Measurements)"
)

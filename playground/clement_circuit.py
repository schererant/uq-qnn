...
class MemristorCircuit:
    ...
    def __init__(
        self, num_modes=3, initial_state=[(1, 0, 0)], stddev=0.01, mean=0.0, config=None
    ):
        """
        ...
        if config is not None:
            ...
            self.memristor_positions = config.get(
                "memristor_positions", None
        ...
        self.init_state = MemristorCircuit.fock_basis(
            self.initial_state, self.num_photons + 1, self.num_modes
        )[0]
        ...
        self.engine = sf.Engine(
            "tf",
            backend_options={"cutoff_dim": self.num_photons + 1, "batch_size": None},
        )
    ...
    def build_circuit(self, initial_phases):
        ...
        if num_phase_values > 0 and num_phase_values % 2 != 0:
            raise ValueError(
                "The number of phase values must be even (pairs for theta and phi for each gate)."
            )
        ...
        if num_gates_from_phases > len(self.structure):
            print(
                "Warning: More gates specified in initial phases than available in the circuit structure based on num_modes."
            )
        ...
        with self.program.context as q:
            ...
            for p in range(num_gates_from_phases):
                if p < len(self.structure):
                    ...
                    MZgate(phases_array[p, 0], phases_array[p, 1]) | (
                        q[modes[0]],
                        q[modes[1]],
                    )
                else:
                    ...
                    print(
                        f"Extra gate at index {p} ignored due to insufficient memristor positions."
                    )

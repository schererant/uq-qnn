        ...
        with tf.GradientTape() as tape:
            ...
            for i in tqdm(
                ...
                if i == 0:
                    ...
                    memristor_circuit = MemristorCircuit(
                        phase1, memristor_phase, phase3, encoded_phases[i]
                    )
                ...
                memory_p3 = tf.tensor_scatter_nd_update(
                    memory_p3, [[time_step % memory_depth]], [prob_state_100]
                )
            ...
            optimizer.apply_gradients(
                zip(gradients, [phase1, phase3, memristor_weight])
            )
...
def predict_mve_memristor(
    ...
    * https://ieeexplore.ieee.org/document/374138
    ...
    for sample in sample_pbar:
        ...
        for i in phase_pbar:
            ...
            logger.log_prediction_step(
                i, loss, phase1_sample, phase3_sample, memristor_weight
            )
    ...
    if stochastic:
        ...
        predictive_uncertainty = np.std(
            np.pow(epistemic_uncertainty, 2) + np.pow(aleatoric_uncertainty, 2)
        )
...
def train_mve_mega_memristor(
    ...
    * https://ieeexplore.ieee.org/document/374138
    ...
    logger.log_initial_training_phase(
        phase1, phase3, memristor_weight, phase4, phase6, memristor2_weight
    )
        ...
        with tf.GradientTape() as tape:
            ...
            for i in tqdm(
                ...
                if i == 0:
                    ...
                    # Init Memristor Circuit
                    memristor_circuit = MemristorMegaCircuit(
                        phase1,
                        memristor_phase,
                        phase3,
                        phase4,
            ...
            gradients = tape.gradient(
                loss,
                [phase1, phase3, memristor_weight, phase4, memristor2_weight, phase6],
            )
            optimizer.apply_gradients(
                zip(
                    gradients,
...
def predict_mve_mega_memristor(
    X_test: np.ndarray,
    y_test: np.ndarray,
    memory_depth: int,
    phase1: float,

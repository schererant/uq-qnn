...
def NLLloss(y_train, prob_state_001, prob_state_010):
    ...
    eps = 0.0001
    loss = tf.fraction(1, tf.pow(prob_state_010, 2) + eps) * tf.square(
        tf.abs(y_train - prob_state_001)
    ) + tf.log(tf.pow(prob_state_010, 2) + eps)
...
def train_megabigmemristor(
    ...
    logger.log_initial_training_phase(
    ...
    for step in pbar:
        ...
        with tf.GradientTape() as tape:
            ...
            for i in tqdm(
                ...
                # print(prob)
                prob_state_000100 = tf.cast(
                    tf.math.real(prob[0, 0, 0, 1, 0, 0]), dtype=tf.float64
                )
                # prob_state_001000 can be used for training with NLL
                prob_state_001000 = tf.cast(
                    tf.math.real(prob[0, 0, 1, 0, 0, 0]), dtype=tf.float64
                )
                prob_state_010000 = tf.cast(
                    tf.math.real(prob[0, 1, 0, 0, 0, 0]), dtype=tf.float64
                )
                prob_state_100000 = tf.cast(
                    tf.math.real(prob[1, 0, 0, 0, 0, 0]), dtype=tf.float64
                )
                prob_state_000010 = tf.cast(
                    tf.math.real(prob[0, 0, 0, 0, 1, 0]), dtype=tf.float64
                )
                prob_state_000001 = tf.cast(
                    tf.math.real(prob[0, 0, 0, 0, 0, 1]), dtype=tf.float64
                )

                # Update memory variables
                memory_p3 = tf.tensor_scatter_nd_update(
                    memory_p3, [[time_step % memory_depth]], [prob_state_000001]
                )
                memory_p5 = tf.tensor_scatter_nd_update(
                    memory_p5, [[time_step % memory_depth]], [prob_state_100000]
                )
                memory_p6 = tf.tensor_scatter_nd_update(
                    memory_p6, [[time_step % memory_depth]], [prob_state_000010]
                )
                memory_p7 = tf.tensor_scatter_nd_update(
                    memory_p7, [[time_step % memory_depth]], [prob_state_010000]
                )
            ...
            logger.log_training_step(
                step, loss, phase1, phase2, memristor_weight
            )  # phase4, phase8, phase9, phase10, phase11, phase12,

            res_mem[("loss", "tr", step)] = [
                loss.numpy()
            ]  # , phase1.numpy(), phase2.numpy(), phase4.numpy(), phase8.numpy(), phase9.numpy(), phase10.numpy(), phase11.numpy(), phase12.numpy(), memristor_weight.numpy()]
    ...
    plot_training_results(
        res_mem, f"{logger.base_dir}/plots/training_results_+{param_id}.png"
    )
...
def predict_megabigmemristor(
    ...
    for sample in sample_pbar:
        ...
        for i in phase_pbar:
            ...
            if i == 0:
                ...
                phase3 = tf.acos(
                    tf.sqrt(memristor_weight * tf.reduce_sum(memory_p3) / memory_depth)
                )
                phase5 = tf.acos(
                    tf.sqrt(memristor_weight * tf.reduce_sum(memory_p5) / memory_depth)
                )
                phase6 = tf.acos(
                    tf.sqrt(memristor_weight * tf.reduce_sum(memory_p6) / memory_depth)
                )
                phase7 = tf.acos(
                    tf.sqrt(memristor_weight * tf.reduce_sum(memory_p7) / memory_depth)
                )
            ...
            logger.log_prediction_step(
                i, loss
            )  # phase1_sample, phase3_sample, memristor_weight)
    ...
    if stochastic:
        ...
        plot_predictions_new(
            ...
            f"{logger.base_dir}/plots/prediction_results_sample{samples}_{param_id}.png",
        )
    else:
        ...
        plot_predictions_new(
            ...
            final_predictions,
            predictive_uncertainty,
            f"{logger.base_dir}/plots/prediction_results_deterministic_{param_id}.png",
        )

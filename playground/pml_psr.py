...
def run_simulation_sequence(params, encoded_phases_all, memory_depth):
    ...
    for i in range(num_samples):
        ...
        circuit = build_perceval_memristor_circuit(
            phase1, memristor_phase, phase3, encoded_phases_all[i]
        )
        proc = pcvl.Processor(backend, circuit)
        proc.with_input(input_state)
        # Use Analyzer for exact probabilities
        analyzer = pcvl.algorithm.Analyzer(
            proc, input_states=[input_state], output_states=[state_001, state_010]
        )
...
def compute_gradients_combined(params, encoded_phases_all, y_train, memory_depth):
    ...
    loss_p1_plus = objective_function(
        params_p1_plus, encoded_phases_all, y_train, memory_depth
    )
    loss_p1_minus = objective_function(
        params_p1_minus, encoded_phases_all, y_train, memory_depth
    )
    ...
    loss_p3_plus = objective_function(
        params_p3_plus, encoded_phases_all, y_train, memory_depth
    )
    loss_p3_minus = objective_function(
        params_p3_minus, encoded_phases_all, y_train, memory_depth
    )
    ...
    loss_mw_plus = objective_function(
        params_mw_plus, encoded_phases_all, y_train, memory_depth
    )
    loss_mw_minus = objective_function(
        params_mw_minus, encoded_phases_all, y_train, memory_depth
    )
...
def train_memristor(
    ...
    params = np.array(
        [phase1_init, phase3_init, memristor_weight_init], dtype=np.float64
    )
    ...
    encoded_phases_all = 2 * np.arccos(
        np.clip(X_train, -1.0 + 1e-9, 1.0 - 1e-9)
    )  # Clip for domain safety
    ...
    for step in pbar:
        ...
        gradients = compute_gradients_combined(
            params, encoded_phases_all, y_train, memory_depth
        )
        ...
        if (
            step % 10 == 0 or step == training_steps - 1
        ):  # Example: Calculate loss every 10 steps
            ...
            if step > 0:
                last_loss = res_mem[
                    ("loss", "tr", max(k[2] for k in res_mem if k[0] == "loss"))
                ][0]
    ...
    if plot and plot_path:
        ...
        plot_save_path = os.path.join(
            plot_path, f"training_results_perceval_{param_id}.png"
        )
        try:
...
def predict_memristor_perceval(
    X_test: np.ndarray,
    y_test: np.ndarray,
    memory_depth: int,
    phase1: float,
    ...
    for sample in sample_pbar:
        if stochastic:
            ...
            phase1_sample = np.random.normal(
                phase1, np.sqrt(var)
            )  # var is variance, stddev is sqrt(var)
            phase3_sample = np.random.normal(phase3, np.sqrt(var))
            # Weight sampling might need clipping if variance is large
            memristor_weight_sample = np.clip(
                np.random.normal(memristor_weight, np.sqrt(var)), 0.01, 1.0
            )
        else:
        ...
        current_params = np.array(
            [phase1_sample, phase3_sample, memristor_weight_sample]
        )

        # Run the simulation sequence for this sample
        sample_preds = run_simulation_sequence(
            current_params, encoded_phases_all, memory_depth
        )
    ...
    all_predictions_np = np.array(
        all_predictions_np
    )  # Shape: (samples, num_test_points)
    ...
    if stochastic:
        ...
        plot_predictions_new(
            X_test, targets, final_predictions, predictive_uncertainty, plot_save_path
        )  # Adapt plot call if needed
    else:

...
engine = sf.Engine(
    "tf", backend_options={"cutoff_dim": num_photons + 1, "batch_size": None}
)
...
para_names = [
    "theta",
    "phi",
]  # Define the parameter names - theta for internal phaseshifter and phi for external phaseshifter
...
phi = tf.random.normal(
    shape=[num_parameters // len(para_names)], stddev=STDDEV, mean=MEAN
)
theta = tf.random.normal(
    shape=[num_parameters // len(para_names)], stddev=STDDEV, mean=MEAN
)

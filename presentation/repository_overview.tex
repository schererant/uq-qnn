...
\begin{enumerate}[leftmargin=*]
    \item \textbf{Modeling and simulation}: Clements circuits, memristive feedback, singles readout and \textbf{N-fold postselected coincidence} over \texttt{working\_detectors} (\(C(W,N)\) channels with \(N=\sum\texttt{input\_state}\)), NumPy and Perceval backends.
...
Work targeted a key bottleneck: memristive NumPy simulations were correct but slow because they rebuilt full unitaries at every step. An optimized state-propagation path for memristive singles runs was added, first for discrete mode and then for swipe mode. Several reruns focused on memristive circuits.

\subsection{Stage 7: Occupation-vector \texttt{input\_state} and N-fold coincidence (breaking)}
Training-facing \texttt{SimConfig} and \texttt{PhotonicCircuit} now share a single convention: \texttt{input\_state} is a \textbf{length-\texttt{n\_modes} occupation vector} (non-negative integers summing to the photon number). Legacy length-1/2 ``mode index lists'' for injection are rejected with a migration hint.

Coincidence mode is no longer limited to a two-photon pair embedded in an \(n(n-1)/2\) layout. The stack uses \textbf{N-fold channels}---lexicographic combinations of distinct detectors in \texttt{working\_detectors}---with postselection semantics aligned between the vectorized NumPy path (Ryser permanents, with a fast closed form when \(N=2\) and the input is two photons in distinct modes) and Perceval SLOS. Regression uses an \(N\)-tuple \texttt{target\_mode} of detector indices; coincidence classification expects \texttt{n\_classes} \(= C(W,N)\). PSR gradient counts follow \(2\sum\texttt{input\_state}\) per phase. \texttt{PhotonicCircuit} is constructed with a \texttt{CircuitConfig} carrying the same occupation and encoding slot metadata.
...
\begin{itemize}[leftmargin=*]
    ...
    \item Added \texttt{PhotonicCircuit} (with \texttt{CircuitConfig}) as a lightweight interface for fast NumPy simulations and N-fold coincidence distributions (\texttt{coincidences(..., detector\_mode="click"|"pnr")}).
